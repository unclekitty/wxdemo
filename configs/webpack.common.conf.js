...
const webEntry = { index: path.resolve('src', 'entry.js') };
const weexEntry = { index: path.resolve('src', 'entry.js') };
...
const getEntryFile = (dir) => {
  ...
  fs.readdirSync(directory).forEach((file) => {
    ...
    else if (stat.isDirectory() && ['build','include','assets','components','router','store', 'utils'].indexOf(file) == -1) {
